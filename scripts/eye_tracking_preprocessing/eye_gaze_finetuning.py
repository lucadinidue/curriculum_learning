...
def train_model(args,train_dataset, data_collator):
    ...
    model = BertForMultitaskTokenClassification.from_pretrained(args.model_path, config=config)
...
def main():
    ...
    parser.add_argument('-u', '--user_id', type=int, default=57)
    ...
    dataset_path = f'data/eye_tracking_data/meco_users/it_{args.user_id}.csv'

    user_dataset, data_collator = preprocess_dataset(dataset_path)

...
def main():
    ...
    model_str = '_'.join(args.model_path.split('/')[-2:])
    output_dir = os.path.join('data/probing_results', model_str)

    if not os.path.exists(output_dir):
        os.makedirs(os.path.join(output_dir, 'train'))
        os.makedirs(os.path.join(output_dir, 'test'))
        save_dataset_representations(args.model_path, args.tokenizer_path, args.train_path, args.test_path, output_dir, args.batch_size)

    train_df = load_dataframe(args.train_path)
    ...
    for feat in features:
        ...
        for layer in range(2):
            ...
            save_predictions(os.path.join(output_dir, f'{feat}.tsv'), predictions, test_labels)

            print(feat, '=',regressor.score(X_test, test_labels))
